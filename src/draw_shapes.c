...
{
	draw_rectangle(img, 125, 250, 200, 12, add_shade(0.0, 0x00FFFF));
...
// // int	gradient(int startcol, int endcol, int position)
// // {
// // 	int r_increment;
// // 	int g_increment;
// // 	int b_increment;
...
// //     r_increment = floor((er - sr) / 4);
// // 	g_increment = floor((eg - sg) / 4);
// // 	b_increment = floor((eb - sb) / 4);

// //     new[0] = (R(startcolor)) + ft_round(increment[0]);
